/**
 * Superpowers-Fusion plugin for OpenCode.ai
 *
...
export const SuperpowersPlugin = async ({ client, directory }) => {
    ...
    // Derive superpowers-fusion skills dir from plugin location
...
- Superpowers-fusion skills: \`superpowers:skill-name\`
- Project skills override personal, which override superpowers-fusion when names match`;
...
# ${description || ''}
# Skill-specific tools and reference files live in ${skillDirectory}
# ============================================`;

                    // Insert as user message with noReply for persistence
                    try {
                        await client.session.prompt({
            ...
            find_skills: tool({
                description: 'List all available skills in the project, personal, and superpowers-fusion skill libraries.',
                args: {},
                execute: async (args, context) => {
                    ...
                    if (allSkills.length === 0) {
                        return 'No skills found. Install superpowers-fusion skills or add project skills to .opencode/skills/';
                    }
        ...
        event: async ({ event }) => {
            ...
            // Re-inject bootstrap after context compaction (compact version)
